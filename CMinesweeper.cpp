...
//
//-----------------------------------------------------------------------
bool CMinesweeper::Update(vector<SVector2D> &mines, vector<SVector2D> &people)
{
	...
	SVector2D vClosestPeople = GetClosestPeople(people);
  
	//normalise it
  Vec2DNormalize(vClosestMine);
  Vec2DNormalize(vClosestPeople);
  ...
  double dot2 = Vec2DDot(m_vLookAt, vClosestPeople);

  //calculate sign
  int sign2 = Vec2DSign(m_vLookAt, vClosestPeople);
...
SVector2D CMinesweeper::GetClosestPeople(vector<SVector2D> &people)
{
	double			closest_so_far = 99999;
	...
	{
		double len_to_object = Vec2DLength(people[i] - m_vPosition);

		if (len_to_object < closest_so_far)
		{
			closest_so_far = len_to_object;

			vClosestObject = m_vPosition - people[i];
...
int CMinesweeper::CheckForPeople(vector<SVector2D> &people, double size)
{
	SVector2D DistToObject = m_vPosition - people[m_iClosestPeople];

	if (Vec2DLength(DistToObject) < (size + 5))
	{
		return m_iClosestPeople;
	}
