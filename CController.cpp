...
const int NumMinePeople = 4;
const SPoint people[NumMineVerts] = { SPoint(-1, -1),
SPoint(-1, 1),
SPoint(1, 1),
SPoint(1, -1) };
...
CController::CController(HWND hwndMain): m_NumSweepers(CParams::iNumSweepers), 
															 ...
															 m_NumPeople(CParams::iNumPeople),
										                     m_hwndMain(hwndMain),
										                     m_iGenerations(0),
                                         ...
                                         cyClient(CParams::WindowHeight)
{
  ...
  vector<int> SplitPoints = m_vecSweepers[0].CalculateSplitPoints();
	...
	//initialize people in random positions within the application window
	for (int i = 0; i<m_NumPeople; ++i)
	{
		m_vecPeople.push_back(SVector2D(RandFloat() * cxClient,
			RandFloat() * cyClient));
	}
	...
	m_OldPen = NULL;
	m_PeoplePen = CreatePen(PS_SOLID, 1, RGB(255, 0, 255));
	...
	for (int i = 0; i<NumMinePeople; ++i)
	{
		m_PeopleVB.push_back(people[i]);
	}
  ...
  //updated appropriately,
	if (m_iTicks++ < CParams::iNumTicks)
	{
		for (int i=0; i<m_NumSweepers; ++i)
		{
			...
			if (!m_vecSweepers[i].Update(m_vecMines, m_vecPeople))
			{
	  ...
	  int GrabHit2 = m_vecSweepers[i].CheckForPeople(m_vecPeople,
				CParams::dMineScale);

			if (GrabHit2 >= 0)
			{
				//we have discovered a people so decrease fitness
				m_vecSweepers[i].DecreasementFitness();

				//people found so replace the mine with another at a random 
				//position
				m_vecPeople[GrabHit2] = SVector2D(RandFloat() * cxClient,
					RandFloat() * cyClient);
			}
...
void CController::Render(HDC surface)
{
	...
	string s2 = "ÁÎÍ¥»Ô 2014141463109";
	TextOut(surface, 5, 15, s2.c_str(), s2.size());
	...
	SelectObject(surface, m_PeoplePen);
	//render the People
		for (int i = 0; i<m_NumPeople; ++i)
		{
			//grab the vertices for the People shape
			vector<SPoint> peopleVB = m_PeopleVB;

			WorldTransform(peopleVB, m_vecPeople[i]);

			//draw the mines
			MoveToEx(surface, (int)peopleVB[0].x, (int)peopleVB[0].y, NULL);

			for (int vert = 1; vert<peopleVB.size(); ++vert)
			{
				LineTo(surface, (int)peopleVB[vert].x, (int)peopleVB[vert].y);
			}

			LineTo(surface, (int)peopleVB[0].x, (int)peopleVB[0].y);
