...
public:
	...
	bool			Update(vector<SVector2D> &mines, vector<SVector2D> &people);
  ...
  SVector2D	GetClosestPeople(vector<SVector2D> &objects);


  //checks to see if the minesweeper has 'collected' a mine
  int       CheckForMine(vector<SVector2D> &mines, double size);
  int CheckForPeople(vector<SVector2D> &people, double size);
	...
	void			        DecreasementFitness(){ m_dFitness-50; }
